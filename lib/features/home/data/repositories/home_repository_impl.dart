...
class HomeRepositoryImpl implements HomeRepository {
  ...
  Stream<Either<Failure, List<CarouselItemEntity>>> getCarouselItems() {
    Log.d('HomeRepositoryImpl: getCarouselItems called');

    return remoteDataSource
        .getCarouselItems()
        .map<Either<Failure, List<CarouselItemEntity>>>((items) {
      Log.d(
          'HomeRepositoryImpl: Received ${items.length} items from data source');
      return Right<Failure, List<CarouselItemEntity>>(items);
    }).transform(StreamTransformer<Either<Failure, List<CarouselItemEntity>>,
            Either<Failure, List<CarouselItemEntity>>>.fromHandlers(
      handleData: (data, sink) => sink.add(data),
      handleError: (error, stackTrace, sink) {
        Log.e('HomeRepositoryImpl: Error occurred: $error');
        sink.add(Left<Failure, List<CarouselItemEntity>>(
          ServerFailure(ErrorStrings.withDetails(
              ErrorStrings.loadCarouselItemsError, error.toString())),
        ));
      },
    ));
  }
